...
*/
void *SPIFFS_Image::getCanvas(void) const {
  if (format != IMAGE_NONE) { // Image allocated?
    if (format == IMAGE_1)
...
*/
ImageReturnCode SPIFFS_ImageReader::drawBMP(char *filename,
                                              Adafruit_SPITFT &tft, int16_t x,
                                              int16_t y, boolean transact, uint32_t bckgrnd_color) {
  ...
  return coreBMP(filename, &tft, tftbuf, x, y, NULL, transact, bckgrnd_color);
}
...
ImageReturnCode SPIFFS_ImageReader::loadBMP(char *filename,
  ...
  return coreBMP(filename, NULL, NULL, 0, 0, &img, false, 0);
}
...
ImageReturnCode SPIFFS_ImageReader::coreBMP(
    ...
    int16_t y,
    SPIFFS_Image *img, // NULL if load-to-screen
    boolean transact,
    uint32_t bckgrnd_color ) {  // SD & TFT sharing bus, use transactions
...
ImageReturnCode SPIFFS_ImageReader::bmpDimensions(char *filename,
  ...
  if ((file = SPIFFS.open(filename, FILE_READ))) { // Open requested file
