...
inline __attribute__((always_inline)) static bool networkIsActive()
{
...
bool ESP32_VS1053_Stream::startDecoder(const uint8_t CS, const uint8_t DCS, const uint8_t DREQ)
{
    ...
    _i2s->begin();
    _i2s->loadDefaultVs1053Patches();
    _i2s->switchToMp3Mode();
    setVolume(_volume);
    return true;
}
...
bool ESP32_VS1053_Stream::connecttohost(const String &url)
{
    ...
    {
    case 206:
        ESP_LOGD(TAG, "server can resume");
    case 200:
    {
    ...
    if (audio_showstreamtitle && data.startsWith("StreamTitle"))
    {
        ...
        if (pos != -1)
        {
            ...
            if (!streamtitle.equals(""))
                audio_showstreamtitle(streamtitle.c_str());
        }
    }
}
...
void ESP32_VS1053_Stream::_handleStream(WiFiClient *const stream)
{
    ...
    while (stream->available() && _i2s->data_request() && _remainingBytes && _musicDataPosition < _metaDataStart)
    {
        ...
        _i2s->playChunk(_vs1053Buffer, c);
...
void ESP32_VS1053_Stream::_handleChunkedStream(WiFiClient *const stream)
{
    ...
    while (_bytesLeftInChunk && _i2s->data_request() && _musicDataPosition < _metaDataStart)
    {
...
void ESP32_VS1053_Stream::loop()
{
    ...
    if (_startMute)
    {
        ...
        {
            _i2s->setVolume(_volume);
            ESP_LOGD(TAG, "startmute ms: %i", WAIT_TIME_MS);
            _startMute = 0;
        }
    }

    if (_http->connected() && _remainingBytes && _i2s->data_request())
    {
        if (_chunkedResponse)
